  ...
  return (
    <div className="pb-24 md:pb-0">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-100 px-6 py-4 flex items-center justify-between md:hidden z-40 shadow-[0_-10px_40px_rgba(0,0,0,0.08)]">
        <button onClick={() => {setActiveTab('home'); setSelectedTaskId(null);}} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === 'home' ? 'text-indigo-600 scale-110' : 'text-slate-400'}`}><i className="fas fa-chart-line text-lg"></i><span className="text-[9px] font-black uppercase tracking-tighter">Stats</span></button>
        <button onClick={() => {setActiveTab('board'); setSelectedTaskId(null);}} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === 'board' ? 'text-indigo-600 scale-110' : 'text-slate-400'}`}><i className="fas fa-search text-lg"></i><span className="text-[9px] font-black uppercase tracking-tighter">Tasks</span></button>
        <button onClick={() => {setActiveTab('my-projects'); setSelectedTaskId(null);}} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === 'my-projects' ? 'text-indigo-600 scale-110' : 'text-slate-400'}`}><i className="fas fa-pen-fancy text-lg"></i><span className="text-[9px] font-black uppercase tracking-tighter">Work</span></button>
